...
B_Crandn = shield_att*(Be_C-(0.5e-6*100)).*(1+(randn(size(Be_C)))*stddev); %Bd(:,1); %SC is flipped, must subtract, adding 1% noise
...
a_tot1 = sqrt(aM.^2+a_TG.^2+a_rgi.^2+a_C.^2+ast1hat.^2);
...
legend('spec performance','total estimate','magnetic','magnetic no shielding','thermal','brownian', 'electrical','stiffness','capacitive sensing','LRI', 'Location','east')
axis([1e-4 1e-1 1e-15 1e-9])
grid on
set(gcf, 'PaperPosition', [0 0 6 5]); %Position the plot further to the left and down. Extend the plot to fill entire paper.
set(gcf, 'PaperSize', [6 5]); %Keep the same paper size
saveas(gcf, 'dragcomp_noise', 'pdf')
...
xlabel('frequency (Hz)')
ylabel('acceleration noise (m/s^{2}Hz^{1/2})')
legend('spec performance','other noise','stiffness','actuation','capacitive sensing','LRI', 'Location','north')
axis([1e-4 1e-1 1e-15 1e-9])
grid on
set(gcf, 'PaperPosition', [0 0 6 5]); %Position the plot further to the left and down. Extend the plot to fill entire paper.
set(gcf, 'PaperSize', [6 5]); %Keep the same paper size
saveas(gcf, 'ndragcomp_noise', 'pdf')
